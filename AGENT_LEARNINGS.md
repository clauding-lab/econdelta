...
## 2026-08-08 — Five months of frozen chart series hid inside a 41-item freshness digest: the seed-without-appender class, and the flash-frozen-as-final trap

**Trigger:** an owner-flagged report that The Brief's monthly charts (remittance, exports, CPI trio) had not moved since March 2026 — verified against `metric_history_monthly` that `remittance_usd_mn_monthly`, `exports_usd_mn_monthly`, `cpi_12m_avg_monthly`, `cpi_p2p_food_monthly`, and `cpi_p2p_nonfood_monthly` all sat at `as_of=2026-03-01` with no `ingested_at` movement since the one-time seed.

**What went wrong:** three compounding failures, each individually plausible.

(a) **Seed-without-appender.** `scripts/seed_macro_monthly.py` back in the day imported a batch of monthly series from a third-party aggregator site (`macro_observer_seed`) as a one-time bootstrap. The site later went dead. No live writer was ever built for these 5 chart-feeding ids specifically — the reserves gross/BPM6 pair (landmine 44, D5) got a live appender when it needed one; these 5 did not. A seed script populating a table looks, at a glance, indistinguishable from an ongoing pipeline — there is no structural signal in `metric_history_monthly` itself that says "this row will never be followed by another."

(b) **Flash-frozen-as-final trap.** The natural-looking fix — derive the monthly finals from EconDelta's own DAILY `monthly_remittance`/`monthly_export` ids, which update far more often — was adversarially proven unsafe before being shipped. Those daily ids are PROVISIONAL FLASH figures (BB revises them before the true monthly close) frozen at 2 decimal places USD bn; treating a flash as a final would have silently written wrong "official" values into a series readers treat as authoritative, trading one kind of staleness for a worse kind of wrongness. The fix instead: (i) derive the CPI trio from EconDelta's OWN daily CPI ids (`general_inflation`/`food_inflation`/`non_food_inflation`), which — unlike the remittance/export flashes — DO carry a trustworthy month-end `source_as_of` (verified 10/12 cells against official BBS figures); and (ii) fetch remittance LIVE from BB's own official monthly wage-remittance webpage table rather than deriving it from any EconDelta-internal id at all.

(c) **41-item digest wallpaper.** The freshness sentinel (`sentinel/`) was already watching `metric_history_monthly` and technically WOULD have flagged all 5 ids as breaches — but its digest listed every breach in one undifferentiated, age-sorted list. A reader-visible chart freezing and an internal parity metric nobody looks at freezing produced identical-looking lines. With 41 total breaches on a bad day, 5 genuinely important ones were statistically invisible in the noise. The sentinel was never "silent" about this — it was drowned out by its own even-handedness.

**Lesson:** A seed script and a live writer look identical in the database; only a companion append-only writer (checked for existence, not assumed) proves the series is still alive. A more-frequent proxy series is not automatically a safer derivation source — "fresher" and "final" are different properties, and conflating them can turn a staleness bug into a correctness bug. And a freshness alert that treats every breach equally will always be dominated by whichever class of metric is most numerous, not whichever class matters most to the humans reading the digest.

**Prevention:** (a) `scripts/backfill_monthly_chart_series.py` (one-time, owner-approved values, hardcoded — never auto-derived) patches the Apr-Jun 2026 gap; `aggregate_latest._write_macro_monthly_append` is the new ongoing appender for the CPI trio + remittance, strictly APPEND-ONLY (checks `metric_history_monthly` before every write so it can never clobber the hand-verified backfill) — see AGENTS.md landmine 50 for the full mechanism. (b) `exports_usd_mn_monthly`/`imports_usd_mn_monthly` have no safe live derivation available yet and are routed to `sentinel.ACCEPTED_STALE_METRIC_IDS` with a dated revisit note, rather than either alerting forever or being silently "fixed" with an unsafe derivation. (c) `sentinel.CHART_FEEDING_METRIC_IDS` (16 ids — every `metric_history_monthly` id a reader-visible chart actually renders) is now a first-class tier; `sentinel/report.py`'s digest lists chart-feeding breaches FIRST under their own heading, so this exact class of freeze can never again hide behind a wall of lower-stakes breaches.

**Hotfix:** PR "fix(monthly): live appender + official backfill for frozen chart series (Phase 1)" — backfill script (15 owner-verified rows), live appender (CPI trio + remittance, both append-only, both with their own failure-isolated try/except and distinct notify messages), sentinel triage (4 new accepted-stale ids + the chart-feeding digest tier).

**Cross-references:** AGENTS.md landmine 50 (the full mechanism), landmine 44 (D5 reserves split — the precedent this PR's append-only pattern and month-end/day-1 `as_of` discipline both follow), landmine 26/47 (`as_of` forgery class — why day-1-of-data-month is non-negotiable here too), landmine 49 (the `general_inflation`/`point_to_point_inflation` wrong-column defect this PR's equality guard defends against), `docs/data-contract.md` §10 (freshness & vintage contract).
...
**What went wrong:** the new LLM-only dict branch accepted any dict whose values were `int`/`float`/`null` and published it unvalidated, on the strength of a comment claiming the deterministic dict branch a few lines above "also skips `validate_value` and trusts per-entry structure instead of a scalar range check" — i.e. that per-entry range-checking already happens structurally somewhere else, so the LLM path didn't need its own. That claim was checked against all three dict-shaped parsers in the repo (`html_call_money.py`, `dse_sector_heat.py`, `pdf_fsr_ownership_cluster.py`) and is false for all three — none range-check. Compounding that: (1) NaN/Infinity pass a bare `isinstance(x, (int, float))` check (both are legitimate floats) and `json.loads`'s default `parse_constant` accepts the literals `NaN`/`Infinity`/`-Infinity`, so a poisoned snapshot would reach `requests`' `allow_nan=False` serializer, which raises and fails the WHOLE day's `metric_history` write — and the same poisoned dict then gets archived to disk with `json.dumps`'s default `allow_nan=True`, re-triggering the same total write failure on every run for up to 60 days via `_load_last_good_snapshot`. (2) Dict KEYS were taken verbatim from the LLM with no allow-list, so a casing drift (`"1d"` instead of `"1D"`) silently defeated `aggregate_latest`'s exact-case `call_money.get("1D")` headline promotion — the same freeze class the dict-fallback fix exists to close, through a different door. (3) The dict branch was unconditional across all 65 `llm_prompt`-configured indicators, so a hallucinated dict on a genuinely scalar money metric (61 of them) would "succeed" as `llm_extracted`, get silently dropped by the Supabase writer's scalar-only filter, and defeat `_terminal_fallback`'s hold-last-good for money — turning a loud stale into a quiet dark.

**Lesson:** "Mirrors the branch above" is not a validation strategy — it is a citation, and citations must be checked against the actual code they cite, not assumed correct because the code compiles and green-tests pass. A comment that asserts a security/correctness property lives *elsewhere* is itself a claim that needs the same verification as a claim about the code in front of you — especially when the "elsewhere" is a less-trusted producer (LLM output) inheriting a guard designed for a more-trusted one (a hand-written HTML/PDF regex parser).

**Prevention:** Any per-key/per-entry loop that skips a scalar validator (`validate_value`, range checks, allow-lists) must state in a comment WHERE the equivalent guard actually lives, and that claim must be grep-verified against the cited file, not assumed from a sibling comment. For dict-shaped LLM extraction specifically: (a) gate on an explicit allow-list of indicator IDs known to be dict-shaped, not "any dict survives"; (b) call `validate_value` per key, which handles NaN/Infinity for free (both compare `False` against any finite range); (c) match keys case-insensitively against the SAME allow-list the deterministic parser trusts, remapped to canonical casing, dropping (not publishing) anything that doesn't match.

**Hotfix:** `parsers/hybrid.py`'s dict branch now: rejects a dict entirely for any indicator not in `_DICT_SHAPED_LLM_INDICATOR_IDS` (`call_money_rate`, `dse_sector_heat`); matches keys case-insensitively against `_expected_dict_keys()` (reusing `html_call_money._TENORS` / `dse_sector_heat._load_taxonomy()`) and remaps to canonical casing; calls `validate_value(value_type=..., valid_range=...)` per non-null key, which also rejects NaN/Infinity and bool; rejects the whole snapshot (routes to `_terminal_fallback`/hold-last-good) if every key was unrecognized or a single value failed validation — a response containing one impossible number is not trusted for the rest. Corrected the false "structural guard" claim in both the deterministic-branch comment and the LLM-branch comment. 8 new tests added (`tests/test_hybrid.py`): out-of-range tenor, negative tenor, NaN tenor, Infinity tenor, unexpected/injection-shaped key dropped, casing-drift key normalized to canonical, all-keys-unrecognized rejected, dict rejected for a scalar indicator (`monthly_remittance`).

**Cross-references:** AGENTS.md landmine 45 (case-fold, exact-match convention this reuses), the 2026-08-08 entry directly below (PR #107→#121, the fix this hardens), `claude_max/validators.py::validate_value` (the existing scalar guard, now reused per-key).
...
## 2026-08-08 — A Pillow security bump silently dragged pdfplumber/pdfminer.six along, and a merged manifest fix does not deploy itself

**Trigger:** adversarial review of PR #118 (`fix/pillow-dependabot-high-alerts`), comparing two CI logs on the same runner image — `main`'s resolve vs. the PR branch's resolve.

**What went wrong:** two stacked gaps.

(a) **An upper bound is itself a vulnerability, and removing it can move more than the package it names.** `requirements.txt`/`pyproject.toml` pinned `Pillow>=10,<12` — the `<12` ceiling, not a stale floor, was the defect: it capped installs below every open Dependabot alert's first-patched version (12.2.0 / 12.3.0), so `pip install` could never resolve a fixed release regardless of how new pip or the index was. Bumping to `Pillow>=12.3,<13` fixed that, but pdfplumber 0.11.10 declares `Pillow>=12.2.0` (0.11.9 only needs `Pillow>=9.1`), so once the Pillow floor cleared 12.2.0, pip's resolver was free to pick the newer pdfplumber — and did, pulling `pdfminer.six==20260107` (pdfplumber pins it exactly) and a newer `pypdfium2` along for the ride. Neither manifest's `pdfplumber` LINE changed in the diff, so the PR body's true statement "no other manifest entries touched" was misleading about the resolved environment: `pdfminer.six` is the actual PDF text-extraction engine every Tier-1 deterministic parser (`pdf_table_row`, `pdf_mfr_row`, `pdf_component`, …) regexes against, and it is explicitly on VISION.md's sign-off list via pdfplumber ("any bump of: playwright, playwright-stealth, pdfplumber, pydantic").

(b) **A merged manifest change is not a deployed change.** `deploy/gitpull.sh` (the only automated deploy step, `econdelta-gitpull.timer` 01:00 BDT) runs `git pull --ff-only` and alerts only when `deploy/*.service`/`*.timer`/`install.sh` changed — it never runs `pip install`, and `deploy/README.md` documents the install step as manual ("if requirements changed"). A `requirements.txt`/`pyproject.toml`-only PR therefore merges, GitHub's Dependabot scanner reads the manifest and closes all 26 alerts, and the ExonVPS `.venv` keeps running the vulnerable Pillow until someone manually SSHes in and reinstalls — green dashboard, vulnerable runtime, no automated signal of the gap either way.

**Lesson:** (1) When a dependency's stated fix is "raise the floor," verify what ELSE the resolver is now free to pick — check the full resolved graph (`pip install --dry-run` / two CI logs diffed), not just the line you edited, especially for any package on the project's own sign-off list. (2) A manifest-only PR is not a deploy; if the repo's deploy path doesn't auto-`pip install` on manifest changes, the PR body must say so explicitly and name the manual step + a verification command, or the fix is "merged" without ever being "live."

**Prevention:** `tests/test_pdfplumber_pillow_coupling.py` pins both manifests' `pdfplumber` specifier below 0.11.10 (the version whose Pillow floor unlocks the newer pdfminer.six/pypdfium2) so this exact silent drift fails CI instead of shipping quietly; raising that cap requires touching the guard, which is the point at which someone has to notice and ask for sign-off. `tests/test_pillow_dependabot_pin.py` was also fixed in the same round: it previously asserted the pin still admits exactly the current patched version, which would have failed CI on the NEXT legitimate security bump (e.g. `Pillow>=13.1,<14` correctly stops admitting 12.3.0) — the lower-bound check alone is the correct, non-self-defeating rule.

**Hotfix:** `pdfplumber` capped at `>=0.11,<0.11.10` in both manifests (keeps the pre-existing 0.11.9/pdfminer.six==20251230 resolution unchanged while Pillow ships); `test_pdfplumber_pillow_coupling.py` added; `test_pillow_dependabot_pin.py`'s self-defeating upper-bound assertions removed plus three cheap robustness fixes (canonicalize the package name so a future `pillow-heif`/`pillow-simd` neighbour can't be prefix-matched, strip inline `#` comments before `Requirement()` parsing, tolerate a wildcard `==12.3.*` version string). The deploy-drift gap (b) was NOT fixed — `deploy/gitpull.sh`/`install.sh` are release-pipeline files needing separate sign-off (VISION.md) — instead the PR body carries an explicit manual post-merge step and verification command.

**Cross-references:** PR #118 (`fix/pillow-dependabot-high-alerts`), VISION.md "Needs Sign-Off" (pdfplumber bump), AGENTS.md landmine 31 (Context7-before-library-work — applies to pdfplumber whenever its cap is eventually raised), `deploy/gitpull.sh`/`deploy/README.md` (manual "if requirements changed" install step).
...
**What went wrong:** BB's MEI PDF prints CPI inflation in ONE table with TWO methodology groups side by side — "Twelve-month average" and "Point to Point" — each carrying its own General/Food/Non-food sub-columns, so the literal string "General" (or "Food", or "Non-food") appears TWICE per row, once under each methodology. `general_inflation`'s config task ("Go to page 15 of the doc, first table") names the row concept ("General") but never the methodology group, so the LLM extraction had no way to know which of the two "General" columns was meant. Checked against 5 recent production rows: `general_inflation` tracked the Twelve-month-average column on 4 of 5 dates (8.59, 8.60, 8.63, 8.60 — matching that column's real April/May values in the fixture almost exactly) and only matched `point_to_point_inflation`'s value (9.16) on the newest row — and that convergence was incidental: the newest row came from config-conversion batch 1's brand-new deterministic `pdf_component` parser (reads an unambiguous Executive Summary sentence), not from `general_inflation`'s own extraction becoming reliable. Sibling indicators `food_inflation`/`non_food_inflation` share the identical config task template and, empirically, DID reliably resolve to the Point-to-Point Food/Non-food columns across the same history — so this wasn't a generic "the LLM always picks the first column-group" failure; it was specific to how the word "General" reads in context, and nobody had checked until this task compared the actual numbers against the actual table.

**Lesson:** When a source document names the SAME leaf label under two different methodology/category groups (here: "General" appears once under "12-month average" and once under "Point to Point"), an LLM extraction instruction that only names the leaf label is genuinely ambiguous, even when a human reading the table would find it obvious from context. The failure is silent — every extracted value looks plausible on its own (it IS a real number from the real table, just the wrong column-group) — so `validate_value`'s range check cannot catch it, and the metric can drift between two real-world quantities for months with no error, no alert, and a metric_history series that reads as "slightly noisy" rather than "structurally confused." The only way this surfaced was a controller-owned comparison of the CANDIDATE duplicate pair's actual row-by-row values against each other and against the source table's own column values — not trusting that "looks like the same concept" or "config task looks similar to a working sibling" is sufficient.

**Prevention:** For any table with multiple methodology/category groups repeating the same column labels, the extraction instruction (LLM prompt OR a future deterministic parser's `row=`/`col=` grammar) MUST name the full column-group path, not just the leaf label — e.g. `"Point to Point → General"`, not just `"General"`. Before treating two similarly-named metric_ids as a duplicate-alias candidate, pull their last 5-10 rows from `metric_history` and diff them line by line; do not infer duplicate-ness from name similarity or a single matching row alone (the newest row here matched purely by coincidence of an unrelated, simultaneous conversion).

**Hotfix:** Not converted — `general_inflation` stays LLM-only (same dynamic-month-row blocker as `food_inflation`/`non_food_inflation`, AGENTS.md landmine 49) until that table gets proper deterministic support. Which column to anchor it to once it IS built is an OWNER decision, not shipped here: anchoring to Point-to-Point General would make it a genuine duplicate of `point_to_point_inflation` (which already reads exactly that number) and would break `general_inflation`'s own series continuity (its history tracked the Twelve-month-average column) with no backfill plan; anchoring to Twelve-month-average General keeps it a distinct, separately-published BB concept and continuous with its own history. A third candidate in the same page window (the Wage Rate Index table's own "Point to Point → General" column, June growth 8.18) must also be ruled out either way before an anchor is picked. Documented as an unresolved, non-alias pair in `docs/data-contract.md` §10.7 rather than silently left unexamined.

**Cross-references:** AGENTS.md landmine 49, `docs/data-contract.md` §10.7 (fifth pair), issue #113, config-conversion batch-1 PR (`pdf_component` for `point_to_point_inflation`, the source of the incidental 2026-06-30 convergence).

## 2026-08-05 — The media screen ran green and empty for 62 consecutive nights; a forged "today" timestamp made every fresher press story compare as stale

**Trigger:** production evidence captured from ExonVPS (`plans/evidence-2026-08-05/media-screen-applog.txt`, `media-screen-journal.txt`). `econdelta-media-screen.service` fired nightly at 21:30 BDT and exited 0 every single night from first deploy (2026-06-04) through 2026-08-04 — 62 consecutive runs. Supabase `media_review` has had ZERO rows, ever (`content-range */0`). Each night's log showed the same shape: ~12 articles collected from tbsnews + thedailystar, then `media screen: 0 candidate(s) inserted, 5-7 skip(s)` — including nights whose collected headlines contained an unambiguous, exactly-the-kind-of-thing-this-screen-exists-to-catch story: `bb-cuts-policy-rate-95-after-2-years` (BB's 2026-07-30 repo-rate cut, 10.00% → 9.50%).

**What went wrong:** two stacked defects, one of them silent-by-omission and the other silent-by-design-tradeoff.

(a) **Invisible sink.** `scrapers/media_screen.py::run_screen` looked up each LLM extraction's free-text `press_name` against the catalog (`by_name.get(ex.indicator_hint.lower())`); on a miss it did a bare `continue` — no skip object, no log line, no way to tell "the LLM correctly found nothing relevant" apart from "the LLM found something but its label didn't string-match our catalog." The nightly "N skip(s)" count therefore only ever covered extractions that BOTH matched a catalog spec AND then failed `classify()` — the gap between "12 collected" and "N skips" was never visible in the logs, every night, for two months. Nobody could see where the other ~5-7 articles' worth of extraction attempts went. NOTE: the historical skip counts in the old logs are also POST-dedup (`_dedup_candidates`/`_dedup_skips`, which were themselves unlogged sinks — see Prevention (1) below), so the exact historical "12 → N → 0" arithmetic is NOT retrospectively reconcilable from the evidence alone. What follows is the mechanism most consistent with the evidence (the real, re-collected rate-cut headline landing on `older-period` every night) — confirmable going forward from the first post-merge night's disposition log, not provable after the fact from the old aggregate counts.

(b) **The actual reason zero press-ahead-of-BB candidates ever fired.** `media_screen/filter.py::classify()`'s Rule 2 decides `fresher_period` vs `older-period` by comparing the press article's stated period against `parsed_as_of` — the `as_of` column of the metric's latest `metric_history` row, read via `_parsed_for()` → `utils.supabase_reader.get_metric_history`. For `policy_rate_repo`, that column has **never** been the rate's true announcement date, across the ENTIRE incident window, via TWO different parsers in sequence: `parsers/pdf_table_column_latest.py` (BB's monthly MEI bulletin, page 10, "Policy rate (repo)" column) fed it for roughly the first 60 of the 62 nights and emits no `source_as_of` at all; `parsers/html_labeled_value.py` (PR #100, 2026-08-03, wired up to fix the *value* going stale) took over for the last ~2 nights and also never emits one — a deliberate choice this time, documented in its own module docstring, because BB's homepage panel carries an unmaintained "Last update" stamp that would date a *current* value five months stale if trusted. Attributing this only to the newer parser is wrong: the forgery predates PR #100 by two months. Neither parser is in `aggregate_latest.py`'s `_NEVER_DATED_PARSE_STRATEGIES` allow-list (`html_table_row`, `html_call_money`, `dse_sector_heat` only) — but membership in that allow-list would not have been a fix either: it only silences the "undated metric" warning inside `_build_source_as_of_map`, it does nothing to the `upsert_metric_history` `as_of=now.date()` fallback that actually stamps the row. Every night, with no `source_as_of` recovered, that fallback wins — `metric_history.as_of` for `policy_rate_repo` is stamped with the aggregate run's OWN calendar date, not the metric's true reporting period, regardless of which parser was live. A press article can only ever cite a PAST date. Comparing a past date against a forged "current" `parsed_as_of` in `classify()`'s Rule 2 lands in `Skip(..., "older-period")` (`media_screen/filter.py:39`) on nearly every input, regardless of how genuinely fresh or conflicting the press figure is. This is the mechanism that best explains why the 2026-07-30 repo-rate-cut headline, re-collected on both 2026-08-02 and 2026-08-04, never once became a Candidate. `call_money_rate` (`html_call_money`, which IS on the allow-list — an accepted, known gap, and equally exposed since allow-list membership was never the mechanism) has the identical exposure. Separately caught in the same review: `aggregate_latest.py`'s comment claiming "every PDF parser DOES already attempt source_as_of recovery" was FALSE — `pdf_table_column_latest`, `pdf_mfr_row`, and `pdf_table_total` recover none; corrected in the same PR (comment-only change).

**Lesson:** a freshness comparison is only as trustworthy as the date it's built on. Before wiring ANY downstream logic (screening, staleness, supersession) to a `metric_history.as_of` value, check what the metric's CURRENT parser actually recovers — not what an allow-list says (allow-list membership silences a warning, it is never itself the fix for an as_of that consumers rely on), and not what a prior parser did (a metric can inherit the same defect across a parser swap, invisibly, if nobody checks the new parser's date-recovery too). Separately: "0 candidates" being written to a `continue` with no accounting is how a real defect hides behind a legitimate-looking "nothing to report" outcome for months — every branch that discards a signal needs either a counted disposition or an explicit "this is fine, here's why" log line; there is no third, silent option. And when reconstructing a past incident from logs that were themselves incomplete (post-dedup, pre-instrumentation), say so explicitly — "consistent with the evidence" is an honest claim, "accounts for all N" is not, when N was never fully logged in the first place.

**Prevention:** (1) `scrapers/media_screen.py` now logs a structured per-article/per-extraction disposition line for every one of the 12 nightly items — collected, no-finding, no-catalog-match, skip(reason), candidate, dedup-drop, already-in-review, or inserted — so this arithmetic is directly readable from `logs/media_screen-systemd.log` going forward, never re-derived from scratch; `_dedup_candidates`/`_dedup_skips` (previously unlogged sinks in their own right — the 2026-08-02 rate-cut headline was collected from BOTH tbsnews and thedailystar the same night, and one copy silently vanished here) now log every drop too, so the final "N skip(s)" summary (itself POST-dedup) reconciles against the per-article lines. (2) A no-catalog-match extraction is now a real `Skip(reason="no-catalog-match")` instead of a silent `continue`, visible in both logs and the Discord digest. (3) A skip on `policy_rate_repo`/`call_money_rate` for `older-period`/`matches-current-data` when `parsed_as_of` is within a day of today now carries an explicit log annotation naming the forged-timestamp mechanism, pointing at AGENTS.md landmine 47 (a same-run-date `==` check would have been dead code — the aggregate stamps `as_of` from an explicit UTC timestamp roughly one full UTC calendar day before media_screen evaluates "today," so the window check is `>=` today−1, not `==` today). (4) `media_screen/streak.py` tracks consecutive zero-insert runs across process restarts (systemd starts a fresh process each run) and fires a Discord warning at 7 consecutive runs, re-firing every 7 thereafter — the tripwire that would have caught this in under two weeks instead of two months; a failure to persist the counter itself now also fires a warning notify, not just a log line, since a silent tripwire failure defeats the point of the tripwire. (5) A silent partial insert (a 2xx response returning fewer ids than candidates posted) now exits 1 + notifies, posting the digest for whichever candidates DID land FIRST so their approve/reject ids aren't orphaned — closes the class of bug landmine 22 named, even though this specific writer was not actually exposed to it. (6) `media_screen/catalog.py`'s `tbill_91d_yield_pct` entry — which had NO `config/sources-v3.json` registry entry, meaning any press mention of it became a guaranteed auto-candidate for a series EconDelta doesn't track — is removed, with a new test (`test_all_catalog_ids_exist_in_registry`) guarding every catalog `metric_id` against the real registry.

**NOT fixed in this PR (needs owner sign-off — it's a scoring/data-currency change, not an observability one):** giving `policy_rate_repo` (and `call_money_rate`) a real `source_as_of`, either by recovering one from a different BB source that IS reliably dated, or by having `media_screen/filter.py::classify()` treat a same-day-forged `parsed_as_of` on an undated-strategy metric as "unknown" rather than "definitely current." Either fix changes what data currency `The Brief`, the staleness sentinel, and this screen all believe about these two metrics — bundled together, not shipped as a side effect of an observability PR.
...
## 2026-08-05 — Two independent "green-looking" systemd failures found in one health pass: a by-design skip counted as a crash, and a root-recreated log file broke nightly logrotate

**Trigger:** a routine box health pass (`systemctl --failed`, log review) the night of 2026-08-05 — not a bug report. Two unrelated units were found in bad states that neither Discord alerts nor `run_logs` had surfaced.

**What went wrong (defect 1 — skip-as-failure):** `econdelta-npl-structure.service` exited 3 — `scrapers/bb_npl_structure.py::main()`'s documented "already captured for this FSR position date — skip" no-op, which `utils/supabase_writer._STATUS_BY_EXIT` maps to `run_logs.status = 'skip'` by design. Systemd's `Restart=on-failure` has no notion of "documented non-zero exit that's actually fine" unless told — so it treated the skip as a crash: the restart counter climbed 1→4 at 5-minute (`RestartSec=300`) intervals across 09:44–10:04 BDT, each retry re-running the full ~50s extraction attempt, until "Start request repeated too quickly" tripped and left the unit sitting in `failed` — polluting `systemctl --failed`, the exact signal a box-health check relies on. Confirmed by grepping every scraper plus `parse_all.py`/`aggregate_latest.py` for `return 3`: `bb_npl_structure.py` is the ONLY `main()` in the whole fleet with a documented skip code; no other unit needed the same fix.

**What went wrong (defect 2 — root-owned log breaks logrotate):** `/etc/logrotate.d/econdelta` failed at midnight with `error opening ".../logs/fiscal_gdp_ratios-systemd.log": Permission denied`. Root cause: every unit writes via `StandardOutput=append:<path>` / `StandardError=append:<path>` under `User=adnan-local` — but if that log file is ABSENT when the unit (re)starts, systemd creates the append target itself, as **root**, before `ExecStart` drops privileges (the service still writes to it fine — it inherited the already-open fd — so nothing here looks broken to the running service or to `run_logs`). The file is now root:root on disk; it stays **world-readable** at 0644 either way, so this is purely a write-side problem — `/etc/logrotate.d/econdelta` runs `su adnan-local adnan-local` (required because the parent `logs/` dir is user-owned) with `copytruncate`, which opens the file O_RDWR to truncate it in place after copying, and a non-root `adnan-local` cannot open a root-owned file for writing — so the WHOLE logrotate run for econdelta exits non-zero. Note the trap only ARMS when the log file is genuinely **absent**: `copytruncate` never removes a log, and `deploy/uninstall.sh` preserves `logs/` across a reinstall, so an already-running unit's existing log doesn't spontaneously flip to this state on an ordinary restart. The practical case that matters is a **newly added unit**, which is born with no log file at all and is therefore born with the trap already armed — which is exactly what the regression test below checks for on every unit, present and future. (The fiscal-gdp unit's log was specifically absent at its 16:54 BDT restart that day — logs are not committed to the repo, so any state that clears an existing log file, not just "a restart," reproduces this.)

**Lesson:** a systemd unit's own log and its own `run_logs` row can both look completely clean while the SURROUNDING infrastructure (the service manager's failure bookkeeping, the OS-level file ownership of a log target) silently degrades — "the payload ran correctly" and "the unit/ops surface reads correctly" are two different claims, and only checking the first misses both of these. Both defects also share a shape: a systemd default (restart-on-any-nonzero-exit; create-append-target-as-whoever-starts-the-unit) that is reasonable in general and wrong for this specific, documented case.

**What went wrong (defect 1, extended — same-day Opus review of the PR fixing this found the class was NOT fully closed):** the fix as first shipped only handled `econdelta-npl-structure.service`'s exit-3 **skip**. But `utils/supabase_writer._STATUS_BY_EXIT` also maps exit 2 to `'stale'` — a documented "anomaly detected, write held/skipped" outcome — and TWO more units had that exact exit code with **no** `RestartPreventExitStatus=`/`SuccessExitStatus=` handling at all: `econdelta-dse.service` (`scrapers/dse_market.py:288`, index/market anomaly vs. the previous trading day) and `econdelta-commodity.service` (`scrapers/commodity_prices.py:161`, price anomaly vs. the previous snapshot). Both could retry-storm identically to the exit-3 case: a deterministic anomaly doesn't change on a retry, so each of the up-to-4 starts inside the unit's own `StartLimit` window would re-fire the same verdict, waste a full scrape, and duplicate the Discord alert, before landing in `failed`. This was NOT a deliberate fleet-wide choice being left in place — `econdelta-forex.service` and `econdelta-briefing.service` already had `RestartPreventExitStatus=2` for their own exit-2 paths, so dse/commodity were simply missed when that pattern was first introduced, not intentionally exempted.

**Prevention:** (a) `SuccessExitStatus=3` added to `econdelta-npl-structure.service` — makes systemd treat the documented skip as a clean exit (`inactive`, not `failed`), so it neither restart-loops nor shows in `--failed`; see AGENTS.md landmine 48 for why this (and not `RestartPreventExitStatus=`) is the correct directive. (a2) `RestartPreventExitStatus=2` added to `econdelta-dse.service` and `econdelta-commodity.service`, mirroring the existing `bb_forex`/`briefing` precedent exactly — these two units still show `failed` (no green pass yet for "stale"; that's a separate, owner-level semantics question tracked as a follow-up, not decided here) but stop retry-storming. (b) every one of the 17 units that write to a persistent log now carries a `+`-privileged `ExecStartPre=+/bin/sh -c '[ -L <path> ] && exit 1; touch <path> && chmod 0644 <path> && chown <User>:<Group> <path>'` immediately before `ExecStart` — `touch` never truncates existing content, `chown` fixes ownership regardless of who most recently (re)created the file, the leading symlink check refuses (aborts the whole start) rather than chowning/chmoding through a swapped symlink (CWE-59), and the `+` prefix runs it outside the unit's own sandboxing (`User=`, `ProtectHome=`, `ProtectSystem=`) so it always has the privilege to do so. This closes the trap at the exact moment it's created, on every start, with no new unit/timer (sidesteps landmine 19 entirely) and no race window (unlike a periodic nightly chown-sweep, which can miss a restart that lands between sweeps). (c) all of this is pinned by tests in `tests/test_deploy_units.py` (`test_npl_structure_treats_documented_skip_exit_as_success`, `test_documented_exit_2_units_do_not_retry_storm` + its `test_exit_2_source_files_still_document_a_stale_exit` sanity check, `test_every_appended_log_has_a_privileged_ownership_guard` — the last one now derives the expected chown owner from each unit's own `User=`/`Group=`, unions `StandardOutput`+`StandardError`, requires `touch`, and treats `file:` the same as `append:`) so a future new unit, an accidental revert, or a wrong-owner guard fails CI instead of surfacing as a 2 a.m. logrotate error or a `--failed` false alarm. Mutation-checked live: removing `RestartPreventExitStatus=2` from a copy of `econdelta-dse.service`, or changing a guard's chown target to `root:root`, each independently flips its pin test from green to red.

**Hotfix:** deploy-hygiene PR #115 (branch `fix/deploy-hygiene`), `deploy/econdelta-npl-structure.service` + all 17 `deploy/econdelta-*.service` files + `tests/test_deploy_units.py` + `deploy/README.md` Troubleshooting section. Deploying to the box is a TARGETED unit-file copy + `daemon-reload` only (AGENTS.md landmine 37) — no new `.timer`, no `install.sh` enable-loop change, no catch-up-fire risk (landmine 5 doesn't apply — no timer schedule changed). Two box-side details worth having on hand at rollout: `daemon-reload` does **not** clear a unit's existing `failed` state — after copying the new `econdelta-npl-structure.service`, also run `sudo systemctl reset-failed econdelta-npl-structure.service`, or it stays red in `systemctl --failed` until its next scheduled fire (Mon 05:29 BDT) quietly clears it. And although the targeted copy is the recommended path, a full `sudo bash deploy/install.sh` is ALSO safe for this specific PR — no new unit is added, so the hardcoded enable-loop (landmine 19) is unchanged, and `systemctl enable --now` on a timer that's already enabled and active is a no-op (no catch-up fire, landmine 5 doesn't apply here either) — worth knowing because `deploy/gitpull.sh` will post exactly that "unit files changed — run install.sh" Discord warning on the first post-merge pull.

**Cross-references:** AGENTS.md landmines 5, 19, 37 (why the box-side rollout stays a targeted copy, not a full `install.sh`), landmine 48 (new — `SuccessExitStatus=` vs `RestartPreventExitStatus=`); `tests/test_deploy_units.py`; `utils/supabase_writer._STATUS_BY_EXIT`; PR #115 (Opus review round, same-day).

## 2026-08-05 — A synthetic test fixture inverted the real column semantics of the exact field under test; the LLM fallback silently became the sole writer for two months

**Trigger:** evidence memo `plans/memos/cab-memo-2026-08-05.md`, written to investigate why `current_account_balance` alternated between -0.301 and -0.778 in Supabase `metric_history`.

**What went wrong:** `current_account_balance`'s deterministic parser (`html_table_row`, `row=Current Account col=2`) was 100% dead code — two stacked defects, either one fatal. (a) BB's Balance of Payments table orders columns prior-FY → current-FY-revised → current-FY-provisional, NOT chronologically, so the hardcoded `col=2` always read LAST year's number. (b) The parser has no unit-conversion step, so even the right cell (in USD million) would have failed `validate_value` against the `amount_usd_bn` range regardless. `validate_value` rejected the deterministic result on every single run, so `hybrid.parse_one` silently fell through to the LLM extract path, whose prompt then contradicted itself ("row=Current Account col=2" vs "return the most recent period's value") — a fresh Sonnet call resolved that contradiction differently run to run, producing the observed alternation. The original test suite (`tests/test_current_account.py`) stayed green throughout because its synthetic fixture used header `Item | FY25 | FY24` — in the FIXTURE, `col=2` genuinely was the current year. On the REAL BB page, `col=2` is the PRIOR year. The fixture inverted the exact semantics of the field the test claimed to verify, and the one test that asserted a value (`result.value == -6500.0`) never called `validate_value` — so the suite proved the parser could extract *a* number from *a* table shaped like BB's, without ever proving it extracted the RIGHT number, or that the number would survive the validation gate production actually enforces. The suite's own docstring admitted the deterministic path was "never the live path" without treating that as the defect it was.

**Lesson:** A test fixture for a data-extraction bug is not just "realistic-looking" — it must be captured from the real producer, because a hand-built fixture reproduces however you *imagine* the source is shaped, and a hardcoded-position bug is invisible against a fixture built with the same (wrong) mental model of column order that produced the bug. Fixtures for money/production-writer metrics must be real captures, and tests must run end-to-end through the actual write-decision function (here, `hybrid.parse_one`), not just the parser — a parser-only test cannot see a `validate_value` rejection, which is precisely the mechanism that hid this for two months.

**Prevention:** (a) `tests/fixtures/bb_bop_2026-08.html` replaces the synthetic fixture — a real page fetched verbatim (`curl -sL`, no challenge encountered, 51,016 bytes, unmodified) with the full 47-row table, all 5 columns, revision suffixes intact; never hand-trimmed. (b) The new dedicated parser (`parsers/bb_bop_row.py`) selects columns by HEADER TEXT (fiscal year + month-window span), never by position, so there is no hardcoded index left to silently point at the wrong year again. (c) Tests assert end-to-end through `parse_one`, including an explicit `validate_value` call on the parser's own output — the exact assertion the old suite never made. (d) A follow-up Opus review of this PR's first draft caught a SECOND instance of the same failure class one layer down: the fixed parser still resolved its header row and data row over a document-wide flat list of `<tr>` elements rather than scoping to a single `<table>`, so a decoy/extra table above the real one could make the column index come from one table while the value came from another — reviewer-proved to produce -1.229 (an in-range, validation-passing, still-wrong value). Fixed by requiring both the row match and the header row to come from the SAME `<table>` element, with a decoy-table regression test pinning the correct real-table value. The pattern repeats at every layer: "shaped like the real thing" is not the same guarantee as "scoped to the real thing."
