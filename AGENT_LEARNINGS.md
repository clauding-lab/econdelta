...
**Hotfix:** Not converted — `general_inflation` stays LLM-only (same dynamic-month-row blocker as `food_inflation`/`non_food_inflation`, AGENTS.md landmine 47) until that table gets proper deterministic support; when it does, anchor it to the Point-to-Point General column specifically. Documented as an unresolved, non-alias pair in `docs/data-contract.md` §10.7 rather than silently left unexamined.

**Cross-references:** AGENTS.md landmine 47, `docs/data-contract.md` §10.7 (fifth pair), issue #113, config-conversion batch-1 PR (`pdf_component` for `point_to_point_inflation`, the source of the incidental 2026-06-30 convergence).
