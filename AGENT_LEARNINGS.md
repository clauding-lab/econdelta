...
(a) **Invisible sink.** `scrapers/media_screen.py::run_screen` looked up each LLM extraction's free-text `press_name` against the catalog (`by_name.get(ex.indicator_hint.lower())`); on a miss it did a bare `continue` — no skip object, no log line, no way to tell "the LLM correctly found nothing relevant" apart from "the LLM found something but its label didn't string-match our catalog." The nightly "N skip(s)" count therefore only ever covered extractions that BOTH matched a catalog spec AND then failed `classify()` — the gap between "12 collected" and "N skips" was completely unaccounted for, every night, for two months. Nobody could see where the other ~5-7 articles' worth of extraction attempts went.

(b) **The actual reason zero press-ahead-of-BB candidates ever fired.** `media_screen/filter.py::classify()`'s Rule 2 decides `fresher_period` vs `older-period` by comparing the press article's stated period against `parsed_as_of` — the `as_of` column of the metric's latest `metric_history` row, read via `_parsed_for()` → `utils.supabase_reader.get_metric_history`. For `policy_rate_repo`, that column is **not** the rate's true announcement date. `parsers/html_labeled_value.py` (the parser wired up in PR #100 to fix the *value* being stale) never emits `source_as_of` in its `ParseResult` — a deliberate choice, documented in its own module docstring, because BB's homepage panel carries an unmaintained "Last update" stamp that would date a *current* value five months stale if trusted. But `aggregate_latest.py`'s `_NEVER_DATED_PARSE_STRATEGIES` allow-list (the set of strategies exempted from the "undated metric" warning) only lists `html_table_row`, `html_call_money`, `dse_sector_heat` — `html_labeled_value` is not in it. So every night, `_build_source_as_of_map` finds no `source_as_of` for `policy_rate_repo`, and `upsert_metric_history` falls back to its global `as_of=now.date()` parameter — the aggregate run's OWN calendar date, not the metric's true reporting period. `metric_history.as_of` for `policy_rate_repo` is therefore stamped "today," every single night, regardless of the real announcement date. A press article can only ever cite a PAST date. Comparing a past date against an artificially-today `parsed_as_of` in `classify()`'s Rule 2 is `ex.period < parsed_as_of` on every possible input except a same-day breaking-news article — landing in `Skip(..., "older-period")` (`media_screen/filter.py:39`) forever, regardless of how genuinely fresh or conflicting the press figure is. This exactly explains why the 2026-07-30 repo-rate-cut headline, re-collected on both 2026-08-02 and 2026-08-04, never once became a Candidate. `call_money_rate` (`html_call_money`, which IS in the allow-list — an accepted, known gap) has the identical exposure by design.

**Lesson:** a freshness comparison is only as trustworthy as the date it's built on. Before wiring ANY downstream logic (screening, staleness, supersession) to a `metric_history.as_of` value, check whether that metric's parser actually recovers a `source_as_of` — an allow-list of "this strategy doesn't date its output" is a landmine unless every consumer of `as_of` re-derives it, and a strategy added later (like `html_labeled_value` in PR #100) can silently fall outside every existing allow-list simultaneously. Separately: "0 candidates" being written to a `continue` with no accounting is how a real defect hides behind a legitimate-looking "nothing to report" outcome for months — every branch that discards a signal needs either a counted disposition or an explicit "this is fine, here's why" log line; there is no third, silent option.

**Prevention:** (1) `scrapers/media_screen.py` now logs a structured per-article/per-extraction disposition line for every one of the 12 nightly items — collected, no-finding, no-catalog-match, skip(reason), candidate, or inserted — so the "account for all 12" arithmetic is directly readable from `logs/econdelta-media-screen-systemd.log` going forward, never re-derived from scratch. (2) A no-catalog-match extraction is now a real `Skip(reason="no-catalog-match")` instead of a silent `continue`, visible in both logs and the Discord digest. (3) A skip on `policy_rate_repo`/`call_money_rate` for `older-period`/`matches-current-data` when `parsed_as_of == today` now carries an explicit log annotation naming the forged-timestamp mechanism, pointing at AGENTS.md landmine 47. (4) `media_screen/streak.py` tracks consecutive zero-insert nights across process restarts (systemd starts a fresh process each run) and fires a Discord warning at 7 consecutive nights, re-firing every 7 thereafter — the tripwire that would have caught this in under two weeks instead of two months. (5) A silent partial insert (a 2xx response returning fewer ids than candidates posted) now exits 1 + notifies, same as a hard write failure — closes the class of bug landmine 22 named, even though this specific writer was not actually exposed to it.
