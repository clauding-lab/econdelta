...
**What went wrong (defect 2 — root-owned log breaks logrotate):** `/etc/logrotate.d/econdelta` failed at midnight with `error opening ".../logs/fiscal_gdp_ratios-systemd.log": Permission denied`. Root cause: every unit writes via `StandardOutput=append:<path>` / `StandardError=append:<path>` under `User=adnan-local` — but if that log file is ABSENT when the unit (re)starts, systemd creates the append target itself, as **root**, before `ExecStart` drops privileges (the service still writes to it fine — it inherited the already-open fd — so nothing here looks broken to the running service or to `run_logs`). The file is now root:root on disk. `/etc/logrotate.d/econdelta` runs `su adnan-local adnan-local` (required because the parent `logs/` dir is user-owned) with `copytruncate`, which needs to open-for-read and truncate the file — neither works for a non-root user against a root-owned file, so the WHOLE logrotate run for econdelta exits non-zero. A 16:54 BDT restart of the fiscal-gdp unit that day recreated its log as root; the file sat that way until the immediate midnight failure was noticed and hand-`chown`'d.
...
**Prevention:** (a) `SuccessExitStatus=3` added to `econdelta-npl-structure.service` — makes systemd treat the documented skip as a clean exit (`inactive`, not `failed`), so it neither restart-loops nor shows in `--failed`; see AGENTS.md landmine 47 for why this (and not `RestartPreventExitStatus=`) is the correct directive. (b) every one of the 17 units that append to a persistent log now carries a `+`-privileged `ExecStartPre=+/bin/sh -c 'touch <path> && chmod 0644 <path> && chown adnan-local:adnan-local <path>'` immediately before `ExecStart` — `touch` never truncates existing content, `chown` fixes ownership regardless of who most recently (re)created the file, and the `+` prefix runs it outside the unit's own sandboxing (`User=`, `ProtectHome=`, `ProtectSystem=`) so it always has the privilege to do so. This closes the trap at the exact moment it's created, on every start, with no new unit/timer (sidesteps landmine 19 entirely) and no race window (unlike a periodic nightly chown-sweep, which can miss a restart that lands between sweeps). (c) both fixes are pinned by new tests in `tests/test_deploy_units.py` (`test_npl_structure_treats_documented_skip_exit_as_success`, `test_every_appended_log_has_a_privileged_ownership_guard`) so a future new unit or an accidental revert fails CI instead of surfacing as a 2 a.m. logrotate error or a `--failed` false alarm.

**Hotfix:** deploy-hygiene PR (branch `fix/deploy-hygiene`), `deploy/econdelta-npl-structure.service` + all 17 `deploy/econdelta-*.service` files + `tests/test_deploy_units.py` + `deploy/README.md` Troubleshooting section. Deploying to the box is a TARGETED unit-file copy + `daemon-reload` only (AGENTS.md landmine 37) — no new `.timer`, no `install.sh` enable-loop change, no catch-up-fire risk (landmine 5 doesn't apply — no timer schedule changed).

**Cross-references:** AGENTS.md landmines 5, 19, 37 (why the box-side rollout stays a targeted copy, not a full `install.sh`), landmine 47 (new — `SuccessExitStatus=` vs `RestartPreventExitStatus=`); `tests/test_deploy_units.py`; `utils/supabase_writer._STATUS_BY_EXIT`.
