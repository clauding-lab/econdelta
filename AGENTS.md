...
33. **Incomplete-TLS-chain hosts (DSE) need the intermediate BUNDLED — never `verify=False`.** DSE's servers (`www.dse.com.bd`, `dsebd.org`) send only the leaf (`CN=*.dsebd.org`), omitting the `Sectigo Public Server Authentication CA DV R36` intermediate. certifi trusts the *root* (`…Root R46`) but `requests`/`urllib3` do NOT AIA-chase the missing intermediate the way browsers do, so every DSE fetch dies `CERTIFICATE_VERIFY_FAILED` (`openssl s_client -connect www.dse.com.bd:443` → `Verify return code: 21`). This froze `dse_market`/`dse_dayend` for ~24 days from 2026-06-11 after DSE's ~2026-06-09 cert renewal — an external cause, not our code. Fix: the intermediate is bundled at `certs/dse_sectigo_r36.pem` and merged with certifi in `utils/ca_bundle.combined_ca_bundle`, which `HttpClient.__init__` sets as `session.verify` (ADDITIVE — never removes a certifi root; degrades to certifi-only on any build failure, so it can't break other hosts). A box-level append of that intermediate to the venv's `certifi/cacert.pem` on ExonVPS + the Mac is the immediate hotfix but is FRAGILE to a `certifi` upgrade (which overwrites `cacert.pem`) — the repo bundle is the durable path; **re-run the append if certifi is ever bumped**. If DSE changes CA, refresh `certs/dse_sectigo_r36.pem` from the leaf's AIA URL (`openssl s_client … | openssl x509 -noout -ext authorityInfoAccess` → the `CA Issuers` URI). NEVER set `verify=False`. (2026-07-09, E1.2.)
