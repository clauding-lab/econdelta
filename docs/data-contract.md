...
**RESOLVED (orchestrator under owner mandate, 2026-08-23; pairing:
the-brief `real_policy_rate` → `point_to_point_inflation`):** the
dynamic-row blocker above was sidestepped rather than solved on the MEI
PDF's own terms — `general_inflation` was repointed away from the PDF
entirely to BB's live `econdata/inflation` HTML page (`parsers/
html_dated_table_row.py`), which presents the two concepts as two
SEPARATE ROWS ("Point to point" / "Monthly Average(Twelve Month)") rather
than two column-groups sharing the "General" leaf label — removing the
ambiguity structurally instead of picking between the two options above.
**Option B is the pairing shipped**: `general_inflation` anchors to the
"Monthly Average(Twelve Month)" row, preserving series continuity with
its own pre-existing history and making it a well-defined, distinct BB-
table series in its own right. `point_to_point_inflation` anchors to the
"Point to point" row on the same page. The third-candidate trap (the Wage
Rate Index table's own "Point to Point → General" column) is moot for
this pair now that neither reads the MEI PDF at all.

This is an OWNER-GATE decision (landmine 49 finding (a) named it as one),
applied here under the repo's standing execute-all-fixes mandate rather
than deferred — **owner veto invited**. The one real-policy-rate
consequence Option B creates (a "real policy rate = nominal repo rate −
CPI" calculation reading against the 12-month-average rather than the
market's current-inflation convention) is handled on the-brief side, not
here: the orchestrator is repointing that specific derivation to read
`point_to_point_inflation` instead of `general_inflation`, preserving the
market-convention reading for that ONE downstream consumer while
`general_inflation` itself stays anchored to the 12-month-average concept
its own history and BB's own table both already agree it is. See AGENTS.md
landmine 52 and the 2026-08-22 AGENT_LEARNINGS.md entry.
