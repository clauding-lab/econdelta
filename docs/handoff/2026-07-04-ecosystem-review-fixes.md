...
### E1.4 — `policy_rate_sdf` writes 7.50 daily while the real SDF floor is 8.50 — RESOLVED (2026-07-10): premise was INVERTED — 7.50 is real, 8.50 was the stale prior

**Resolution (2026-07-10):** The "8.50" premise was wrong. Confirmed cross-repo, on the live YieldScope/The Brief surface, and against the owner's records that **SDF = 7.50 is REAL** — Bangladesh Bank cut the SDF floor; the review's 8.50 was a stale prior. The pipeline value (7.50) was correct all along, and the current corridor **SDF 7.50 < repo 10.00 < SLF 11.50** satisfies the invariant — NO value or parser output was changed. A **corridor coherence guard** (`utils/anomaly.check_corridor_coherence`, invoked in `aggregate_latest.main` once all three legs are assembled in `data`) was added per item (4) below: it verifies SDF ≤ repo ≤ SLF and fires a loud `notify("error", …)` on a genuine ordering violation (silent on any missing leg; detect-and-alert, never rejects the run). See the 2026-07-10 AGENT_LEARNINGS entry. The original investigation notes are retained below for history.
